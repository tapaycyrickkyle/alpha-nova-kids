...
const SUPABASE_IMAGE_FOLDER = "uploads/images/numbers-cards";
const SUPABASE_AUDIO_FOLDER = "uploads/audio/numbers";
...
function NumberCardItem({
  ...
  function handleImageChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    onImageSelected(card.number, file);
    event.target.value = "";
  }
  ...
  function handleAudioChange(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    onAudioSelected(card.number, file);
    event.target.value = "";
  }
    ...
    <>
      ...
      <div className="group flex h-full flex-col rounded-2xl border border-surface-variant/20 bg-surface-container-lowest p-5 shadow-[0px_10px_30px_rgba(44,47,49,0.04)] transition-all hover:-translate-y-1 hover:shadow-[0px_20px_40px_rgba(44,47,49,0.08)] md:p-6">
        <div className="mb-6 grid grid-cols-[56px_1fr_56px] items-center gap-3">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-primary-container/20 text-3xl font-black text-primary">{card.number}</div>
        ...
        <div className="flex flex-1 flex-col gap-4">
          <section className="rounded-2xl border border-surface-variant/20 bg-white/70 p-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-on-surface-variant">Add a visual reference for this number.</p>
            </div>
            ...
            >
              {imageState.previewUrl ? (
                ...
                <>
                  ...
                  <p className="mt-1 max-w-[14rem] text-sm text-on-surface-variant">Use a clear child-friendly image for the number.</p>
                </>
              )}
            </div>
          ...
          <section className="rounded-2xl border border-surface-variant/20 bg-white/70 p-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-on-surface-variant">Upload the MP3 counting clip.</p>
            </div>
              ...
              {audioState.previewUrl ? (
                ...
                <div className="mx-auto flex w-full max-w-[15rem] flex-col items-center text-center">
                  ...
                  <p className="mt-2 text-xs leading-5 text-on-surface-variant sm:text-sm">
                    Add the spoken audio for {card.label.toLowerCase()}.
                  </p>
                </div>
              )}
            </div>
...
export function NumbersUploadGrid({
  cards,
  onPendingImagesChange,
  uploadAllToken = 0,
  onUploadAllComplete,
}: NumbersUploadGridProps) {
  const [imageStates, setImageStates] = useState<Record<string, CardImageState>>(
    () => Object.fromEntries(cards.map((card) => [card.number, createEmptyImageState()])),
  );
  const [audioStates, setAudioStates] = useState<Record<string, CardAudioState>>(
    () => Object.fromEntries(cards.map((card) => [card.number, createEmptyAudioState()])),
  );
  ...
  useEffect(() => {
    ...
    async function loadExistingImages() {
      try {
        const itemKeys = cards.map((card) => sanitizeSegment(card.number));
        const [metadataMap, audioMetadataMap] = await Promise.all([
          fetchImageAssetMetadataMap("numbers", itemKeys),
          fetchAudioAssetMetadataMap("numbers-audio", itemKeys),
        ]);

        await Promise.all(
          cards.map(async (card) => {
            const cardKey = sanitizeSegment(card.number);
            ...
            if (existingFilePath) {
              ...
              setImageStates((current) => {
                const currentState = current[card.number];
  ...
  useEffect(() => {
    async function uploadAllImages() {
      const stagedEntries = cards.filter((card) => {
        const imageState = imageStates[card.number];
        const audioState = audioStates[card.number];
        return (
    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
      {cards.map((card) => (
        <NumberCardItem
          key={card.number}
          card={card}
          imageState={imageStates[card.number] ?? createEmptyImageState()}
          audioState={audioStates[card.number] ?? createEmptyAudioState()}
